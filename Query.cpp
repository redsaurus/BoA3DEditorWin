...
void start_full_data_dump()
{
	...
	FSWrite(data_dump_file_id, &len, (char *) get_text);
		...
		for (out_sec.y = 0; out_sec.y < scenario.out_height ; out_sec.y++) {
			...
			sprintf((char *)get_text,"   Town entrance %d: town %d, top,left = (%d,%d) bottom,right = (%d,%d)\r",i,outdoor.exit_dests[i],outdoor.exit_rects[i].left,outdoor.exit_rects[i].top,outdoor.exit_rects[i].right,outdoor.exit_rects[i].bottom);
			len = (long) (strlen((char *)get_text));
			FSWrite(data_dump_file_id, &len, (char *) get_text);
			}
		}
